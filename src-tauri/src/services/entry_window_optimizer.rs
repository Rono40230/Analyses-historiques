...
pub fn optimize_entry_window(
    ...
    for (_day, day_candles) in candles_by_day.iter() {
        ...
        for offset in 0..15 {
            ...
            if let Some(entry_candle) = sorted_candles.iter().find(|c| c.datetime.minute() as i64 == target_minute) {
                ...
                for future_candle in sorted_candles.iter() {
                    if future_candle.datetime > entry_candle.datetime {
                        ...
                        if move_down > 0.0 { // Drawdown pour un long
