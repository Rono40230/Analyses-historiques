...
use crate::services::session_analyzer::{TradingSession, CalendarCorrelation};
    ...
    pub fn calculate_correlation(
        ...
        for session in sessions {
            ...
            let high_impact_count = Self::count_events(
                &mut conn, start_hour, end_hour, "HIGH", session.name == "Sydney"
            )?;

            let total_events_count = Self::count_events_multiple(
                &mut conn, start_hour, end_hour, vec!["HIGH", "MEDIUM"], session.name == "Sydney"
            )?;
        ...
        if crosses_midnight {
            let count1: i64 = calendar_events::table
                ...
                .filter(diesel::dsl::sql::<diesel::sql_types::Bool>(
                    &format!("cast(strftime('%H', event_time) as integer) >= {}", start_hour)
                ))
                .count()
                .get_result(conn)
                .unwrap_or(0);
            ...
            let count2: i64 = calendar_events::table
                .filter(calendar_events::impact.eq(impact))
                .filter(diesel::dsl::sql::<diesel::sql_types::Bool>(
                    &format!("cast(strftime('%H', event_time) as integer) < {}", end_hour)
                ))
                .count()
                .get_result(conn)
                .unwrap_or(0);
