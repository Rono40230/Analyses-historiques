...
impl<'a> HourlyStatsCalculator<'a> {
    ...
    fn calculate_for_hour(&self, hour: u8, candles: &[&Candle]) -> Result<HourlyStats> {
        ...
        let range_mean = owned_candles
            .iter()
            .map(|c| c.high - c.low)
            .sum::<f64>()
            / owned_candles.len() as f64;
        ...
        let breakout_percentage = (breakout_count as f64 / tr_dist.is_breakout.len() as f64) * 100.0;
